...
// 12 bit sine wave; spans 256 samples 
...
// 12 bit sine wave; spans 100 samples 
...
void loop() {
  // Composing a fast (10hz) signal over a slow (~0.4hz) one (trend) 
  for(int r=0; r<10; r++) {
    sign_raw  = 3.0*(float)sine12bits_256samples[s1i] + sine12bits_100samples[s2i];
    s2i = ++s2i % 99;
    
    // High-pass filter removes low frequency trend 
    sign_filt = fhp.filterIn((float)sign_raw);

    // Loop time has to be approximately equal to selected sampling period
    delay(1); 

    // View with serial plotter 
    Serial.print(sign_raw);
    Serial.print(",");
    Serial.println(sign_filt);
  }
  s1i = ++s1i % 255;
}
